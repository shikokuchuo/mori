...
- **shm.c** — platform SHM create/open/close + finalizers for both sides; `mori_shm_unlink_name` (unlink-by-name, all POSIX) and `mori_shm_reap` (Linux: enumerate `/dev/shm`; macOS: enumerate mori's per-user marker directory `<TMPDIR>/mori`; both share one scan loop in `mori_shm_reap` that classifies by the PID in each name via `mori_pid_alive` and unlinks dead-PID orphans (the scan directory is selected by a platform `#ifdef`); Windows / other POSIX: cannot reap, return `NULL` / `*n == 0`). The macOS POSIX-shm namespace is not enumerable (no `/dev/shm`; invisible to `ipcs`/`lsof` once the creator dies), so each region drops a best-effort empty **marker file** in `mori_shm_create` and removes it in `mori_shm_os_unlink` (the single unlink seam, covering host finalizer / by-name unlink / create-fail). Both platforms share identical reap semantics (semantics A): a dead-PID name is reported as removed only when `shm_unlink` actually reclaimed a region, so a concurrent reap never double-reports; on macOS a stale dead-PID marker (region already gone) is still cleaned but not reported, because `mori_shm_os_unlink` removes the marker before returning. Marker failures only forfeit reapability, never the region. Also the cross-platform error path: `mori_err_classify` (static; native errno / GetLastError → portable `MORI_ERR_*` category) and `mori_err_describe` (category summary + platform remediation hint). `mori_shm_create` / `mori_shm_create_heap` return `MORI_OK` or a failure category directly; the native code is classified **before** any `close`/`unlink`/`CloseHandle` (those clobber `errno` / `GetLastError`).
...
- **macOS**: Kernel-backed SHM via `shm_open` / `shm_unlink` (in libc, no extra link flags). `MAP_POPULATE` is a no-op (defined to 0). The kernel POSIX-shm namespace has no filesystem footprint and cannot be enumerated, so orphan reaping relies on a per-user marker-file registry under `TMPDIR` (resolved via `$TMPDIR`, then `confstr(_CS_DARWIN_USER_TEMP_DIR)`, then `/tmp`) that `mori_shm_reap` scans in place of `/dev/shm`; markers are created and removed in lockstep with the region and are entirely best-effort.
